...
async function cleanupDuplicates() {
    console.log('=== CLEANING UP DUPLICATES IN ORDERS ===\n');

    const ordersSnap = await db.collection('orders').get();
    console.log(`Total docs before cleanup: ${ordersSnap.size}`);

    // Group by orderNumber
    const groups = {};
    ordersSnap.forEach(doc => {
        const data = doc.data();
        const key = (data.orderNumber || data.jobId || '').trim();
        if (!key) return;
        if (!groups[key]) groups[key] = [];
        groups[key].push({ doc, data });
    });

    let deletedCount = 0;
    for (const [orderNum, entries] of Object.entries(groups)) {
        if (entries.length <= 1) continue;

        // Find the "good" entry (with fireId, dateReceived - the CML-synced one)
        const good = entries.find(e => e.data.fireId && e.data.dateReceived);
        if (!good) {
            console.log(`  ⚠️ ${orderNum}: ${entries.length} copies but none with full data, skipping`);
            continue;
        }

        // Delete the others
        for (const entry of entries) {
            if (entry.doc.id === good.doc.id) continue;
            console.log(`  🧹 Deleting duplicate for "${orderNum}": docId=${entry.doc.id} (keeping: ${good.doc.id})`);
    ...
    // Verify
    const afterSnap = await db.collection('orders').get();
    console.log(`Total docs after cleanup: ${afterSnap.size}`);
}
