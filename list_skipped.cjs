...
async function listSkipped() {
    const emailSnap = await db.collection('zakazka_emails').get();
    const ordersSnap = await db.collection('orders').get();
    const existingIds = new Set();
    ordersSnap.forEach(doc => {
        if (doc.data().recoveredFromEmail) existingIds.add(doc.data().recoveredFromEmail);
    });

    console.log('--- SKIPPED EMAILS ---');
    emailSnap.forEach(doc => {
        if (!existingIds.has(doc.id)) {
            console.log(`SKIP|${doc.id}|${doc.data().subject}`);
        }
    });
}

listSkipped().catch(console.error);
