...
async function findDuplicates() {
    try {
        // 1. Check PUBLIC_ORDERS (orders) for duplicates
        console.log('=== ORDERS COLLECTION (zakázka na cestě) ===');
        const ordersSnap = await db.collection('orders').get();
        console.log(`Total docs: ${ordersSnap.size}\n`);

        const ordersByJobId = {};
        ordersSnap.forEach(doc => {
            const data = doc.data();
            const key = (data.jobId || data.orderNumber || '???').trim();
            if (!ordersByJobId[key]) ordersByJobId[key] = [];
            ordersByJobId[key].push({
                docId: doc.id,
                jobId: data.jobId,
        ...
        // Show duplicates
        const dupes = Object.entries(ordersByJobId).filter(([k, v]) => v.length > 1);
        if (dupes.length > 0) {
            console.log(`FOUND ${dupes.length} DUPLICATE JOB IDs:\n`);
            dupes.forEach(([jobId, docs]) => {
                console.log(`  jobId: "${jobId}" (${docs.length} copies)`);
                docs.forEach((d, i) => {
                    console.log(`    [${i}] docId=${d.docId}, fireId=${d.fireId}, dateReceived=${d.dateReceived}, deadline=${d.deadline}`);
                    console.log(`        client=${d.clientName || d.customer}, isTracked=${d.isTracked}, fromQueue=${d.fromQueue}`);
                });
                console.log('');
            });
        } else {
            console.log('No duplicates found in orders collection.');
        }

        // 2. Show all orders for comparison
        console.log('\n=== ALL ORDERS ===');
        ordersSnap.forEach(doc => {
            const d = doc.data();
            console.log(`  docId=${doc.id} | jobId=${d.jobId} | orderNumber=${d.orderNumber} | fireId=${d.fireId} | dateReceived=${d.dateReceived} | deadline=${d.deadline} | isTracked=${d.isTracked}`);
        });

        // 3. Check board cards for comparison
        console.log('\n=== BOARD CARDS ===');
        const boardSnap = await db.collection('cml_board_cards').get();
        console.log(`Total: ${boardSnap.size}`);
        boardSnap.forEach(doc => {
            const d = doc.data();
            console.log(`  docId=${doc.id} | jobId=${d.jobId} | fireId=${d.fireId} | isTracked=${d.isTracked} | fromQueue=${d.fromQueue} | dateReceived=${d.dateReceived}`);
        });
