...
async function checkOrders() {
    try {
        ...
        console.log('\n--- CHECKING "orders_test" COLLECTION ---');
        const testSnap = await db.collection('orders_test').get();
        console.log(`Test Count: ${testSnap.size}`);
        testSnap.forEach(doc => {
            console.log(`[TEST_DATA]|${doc.id}|${JSON.stringify(doc.data())}`);
        });

        console.log('\n--- CHECKING "cml_board_cards" ---');
        const boardSnap = await db.collection('cml_board_cards').get();
        console.log(`Board Count: ${boardSnap.size}`);
        boardSnap.forEach(doc => {
            const data = doc.data();
            console.log(`[BOARD_DATA]|${doc.id}|${data.jobId}|${data.customer}|${data.jobName}|${data.isTracked}`);
        });

        console.log('\n--- CHECKING "zakazka_emails" FOR RECOVERY ---');
        const emailSnap = await db.collection('zakazka_emails').orderBy('received_at', 'desc').get();
        console.log(`Email Count: ${emailSnap.size}`);
        emailSnap.forEach(doc => {
            const data = doc.data();
            console.log(`[RECOVERY_DATA]|${data.zakazka_id}|${data.subject}|${data.customer || data.sender || ""}|${data.received_at}`);
        });
    } catch (e) {
