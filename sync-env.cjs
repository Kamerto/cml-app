const { execSync } = require('child_process');

const envs = {
    'FIREBASE_PROJECT_ID': 'calamarus-907d7',
    'FIREBASE_CLIENT_EMAIL': 'firebase-adminsdk-fbsvc@calamarus-907d7.iam.gserviceaccount.com',
    'FIREBASE_PRIVATE_KEY': '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----'
};

for (const [name, value] of Object.entries(envs)) {
    console.log(`Setting ${name}...`);
    try {
        // Try to remove first if exists
        try { execSync(`npx vercel env rm ${name} production --yes`, { stdio: 'inherit' }); } catch (err) { }

        // Add by piping the value to avoid shell escaping issues with the multiline key
        const cmd = `echo "${value.replace(/"/g, '\\"')}" | npx vercel env add ${name} production`;
        // Actually, echo might still mess up backslashes. Let's use fs to write to a temp file and then add from file.
        const fs = require('fs');
        fs.writeFileSync('temp_val.txt', value);
        execSync(`npx vercel env add ${name} production < temp_val.txt`, { stdio: 'inherit', shell: 'powershell' });
        fs.unlinkSync('temp_val.txt');

        console.log(`✅ ${name} set.`);
    } catch (e) {
        console.error(`❌ Error setting ${name}:`, e.message);
    }
}
