...
async function gapAnalysis() {
    console.log('--- ORDER GAP ANALYSIS ---');
    const emailSnap = await db.collection('zakazka_emails').get();

    const numbers = [];
    emailSnap.forEach(doc => {
        const subject = doc.data().subject || "";
        const matches = subject.matchAll(/(\d{1,4})\/(\d{2})/g);
        for (const match of matches) {
            numbers.push({ full: match[0], n: parseInt(match[1]), y: parseInt(match[2]) });
        }
    });

    // Sort by year then number
    numbers.sort((a, b) => a.y !== b.y ? a.y - b.y : a.n - b.n);
    ...
    numbers.forEach(curr => {
        if (prev && curr.y === prev.y) {
            if (curr.n > prev.n + 1) {
                console.log(` GAP: Missing ${prev.n + 1} to ${curr.n - 1} / ${curr.y}`);
            }
        }
